...
for (let y = 0; y < height; y++) {
  for (let x = 0; x < width; x++) {
    ...
    const innerY = y / scale;
    
    const leftSide = (innerX < cx / scale && innerY > 4 && innerY < height / scale - 4 && 
                      (innerX >= cx / scale - 6 && innerX <= cx / scale - 2) &&
                      (innerY >= 6 && innerY <= height / scale - 6));
    
    const rightSide = (innerX >= cx / scale && innerY > 4 && innerY < height / scale - 4 &&
                       (innerX >= cx / scale && innerX <= cx / scale + 4) &&
                       ((innerY >= 8 && innerY <= 12) || (innerY >= 14 && innerY <= height / scale - 6)));
    
    const bottomTip = (innerX >= cx / scale - 1 && innerX <= cx / scale + 2 && 
                       innerY >= height / scale - 6 && innerY <= height / scale - 3);
